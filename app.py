...
# --- Función Limpiadora Mejorada con tus ubicaciones ---
def limpiar_ubicacion_pro(u):
    u_s = str(u).upper().strip()
    if not u_s or u_s == "NAN": return "SIN DATO"
    
    # Mapeo según tu lista
    ...
    return u_s # Si no está en la lista, deja el nombre original pero limpio
...
def cargar_fallas_gsheet():
    try:
        ...
        # Leemos Columna E (índice 4)
        if len(df.columns) > 4:
            df['Ubicación Equipo'] = df.iloc[:, 4].apply(limpiar_ubicacion_pro)
        else:
...
if df_pedidos is not None:
    try:
        ...
        # Unir Memorias
        df_full = pd.merge(df_base, df_mem_rep, on='ID_Unico', how='left')
        ...
        # --- BUSCADOR ---
        txt_busq = st.text_input("🔍 Buscador General (Filtra equipos o repuestos)", placeholder="Ej: 1661, Chicoral, Filtro").upper()
        ...
        with t1:
            ...
            ed_p = st.data_editor(df_p[['Sel', 'Fecha_Prog'] + cols_v], use_container_width=True, hide_index=True, 
                                  column_config={"Fecha_Prog": st.column_config.DateColumn("📅 Prog"), "Sel": st.column_config.CheckboxColumn(width="small")}, disabled=cols_v)
            if st.button("💾 Guardar"):
                for i, r in ed_p.iterrows(): df_full.loc[df_full['ID_Unico']==df_p.loc[i, 'ID_Unico'], 'Fecha_Prog'] = r['Fecha_Prog']
        ...
        with t4:
            if not df_fv.empty:
                ...
                g2.altair_chart(alt.Chart(df_final_f).mark_bar().encode(x=alt.X('COMPONENTE:N', sort='-y'), y='distinct(Cod Equipo)'), use_container_width=True)
                ...
                ed_f = st.data_editor(df_final_f[['Enviar_Tecnico', 'Cod Equipo', 'COMPONENTE', 'Falla', 'Ubicación Equipo']], use_container_width=True, hide_index=True)
