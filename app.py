...
if df_pedidos is not None:
    try:
        ...
        df_pedidos.rename(columns={
            ...
            df_pedidos.columns[4]: 'UBI_PED', 
            df_pedidos.columns[6]: 'Cod Equipo',
            df_pedidos.columns[8]: 'Cantidad',
            df_pedidos.columns[9]: 'Creación Pedido',     # COL J (Índice 9)
            df_pedidos.columns[11]: 'Cantidad en Inventario', # COL L (Índice 11)
            df_pedidos.columns[17]: 'Fecha_Llegada'
        }, inplace=True)
        
        # Convertir a numérico y fecha
        df_pedidos['Cantidad en Inventario'] = pd.to_numeric(df_pedidos['Cantidad en Inventario'], errors='coerce').fillna(0)
        df_pedidos['Creación Pedido'] = pd.to_datetime(df_pedidos['Creación Pedido'], errors='coerce')
        ...
        m2.metric("🛡️ Reserva", len(df_v[df_v['Estado']=='RESERVA']))
        ...
        # COLUMNAS VISIBLES (Añadidas Cantidad en Inventario y Creación Pedido)
        cols_v = ['Prioridad', 'Cód insumo', 'Producto', 'Cod Equipo', 'Ubicación Equipo', 'Cantidad', 'Cantidad en Inventario', 'Creación Pedido', 'Días en Almacén']
        ...
        cfg = {"Sel": st.column_config.CheckboxColumn(width="small"), 
               ...
               "Creación Pedido": st.column_config.DateColumn("📝 Creación", format="DD/MM/YYYY"),
               "Cantidad": st.column_config.NumberColumn("📦 Pedido", format="%d"),
               "Cantidad en Inventario": st.column_config.NumberColumn("🏢 Stock", format="%d"),
               "Ubicación Equipo": st.column_config.TextColumn("📍 Ubicación", width="medium")}
        ...
        with t1:
            ...
            if not df_p.empty:
                ...
                if not sel.empty:
                    ...
                    if c1.button("✅ Mover a COMPLETADO"): df_full.loc[df_full['ID_Unico'].isin(ids), 'Estado'] = 'COMPLETADO'; guardar_en_github(ARCHIVO_CSV, df_full[['ID_Unico', 'Estado', 'Fecha_Prog']])
                    if c2.button("🛡️ Mover a RESERVA"): df_full.loc[df_full['ID_Unico'].isin(ids), 'Estado'] = 'RESERVA'; guardar_en_github(ARCHIVO_CSV, df_full[['ID_Unico', 'Estado', 'Fecha_Prog']])
        ...
        with t2:
            ...
            if not df_r.empty:
                ...
                if st.button("💾 Guardar Fechas (Reserva)"):
                    for i, r in ed_r.iterrows(): df_full.loc[df_full['ID_Unico']==df_r.loc[i, 'ID_Unico'], 'Fecha_Prog'] = r['Fecha_Prog']
                    guardar_en_github(ARCHIVO_CSV, df_full[['ID_Unico', 'Estado', 'Fecha_Prog']])
                sel_r = ed_r[ed_r['Sel']]
        ...
        with t3:
            ...
            if not df_c.empty:
                ...
                sel_c = ed_c[ed_c['Sel']]
                if not sel_c.empty and st.button("🔙 Devolver a PENDIENTE"): 
                    df_full.loc[df_full['ID_Unico'].isin(df_c.loc[sel_c.index, 'ID_Unico']), 'Estado'] = 'PENDIENTE'
                    guardar_en_github(ARCHIVO_CSV, df_full[['ID_Unico', 'Estado', 'Fecha_Prog']])
        ...
        with t4:
            if not df_fv.empty:
                ...
                ed_f = st.data_editor(df_final_f[['Enviar_Tecnico', 'Cod Equipo', 'COMPONENTE', 'Falla', 'Ubicación Equipo']], column_config={"Enviar_Tecnico": st.column_config.CheckboxColumn("👷 Obra")}, use_container_width=True, hide_index=True)
                if st.button("💾 Guardar Gestión Novedades"):
                    for i, r in ed_f.iterrows(): df_fallas_full.loc[df_fallas_full['ID_Falla']==df_final_f.loc[i, 'ID_Falla'], 'Enviar_Tecnico'] = r['Enviar_Tecnico']
